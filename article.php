			...
			<section id="main-section">
				...
				<div id="buy-container">
					...
					<div class="buy-field" id="article-price">
						...
						<?php if ($article['discount']) : ?>
							<span><?= $article['new_price'] ?> VND</span>
							<span id="old-price"><?= $article['price'] ?> VND</span>
						<?php else : ?>
							<span><?= $article['price'] ?> VND</span>
						<?php endif ?>
					...
					<div class="buy-field" id="article-stock-<?= ($article['stock'] ? 'yes' : 'no') ?>">
						<span class="label">
							<?=
								$article['stock']
									? "Trong kho -  {$article['stock']}  còn lại"
									: 'Out Of Stock'
							?>
						</span>
					</div>
			...
			<section class="detail-section">
				...
				<div class="reviews-container">
					<?php if (!empty($article_reviews)) : ?>
						...
						<?php endforeach ?>
					<?php else : ?>
						<h3>Bài viết này không có bất kỳ đánh giá nào</h3>
					<?php endif ?>
				</div>
				<!-- loged user review -->
